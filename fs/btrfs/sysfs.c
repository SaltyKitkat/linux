...
static ssize_t delayed_block_rsv_size_show(struct kobject *kobj,
				    struct kobj_attribute *ka, char *buf)
{
	struct btrfs_fs_info *fs_info = to_fs_info(kobj->parent);
	struct btrfs_block_rsv *block_rsv = &fs_info->delayed_block_rsv;
	return btrfs_show_u64(&block_rsv->size, &block_rsv->lock, buf);
}
BTRFS_ATTR(allocation, delayed_block_rsv_size, delayed_block_rsv_size_show);

static ssize_t delayed_block_rsv_reserved_show(struct kobject *kobj,
					struct kobj_attribute *a, char *buf)
{
	struct btrfs_fs_info *fs_info = to_fs_info(kobj->parent);
	struct btrfs_block_rsv *block_rsv = &fs_info->delayed_block_rsv;
...
BTRFS_ATTR(allocation, delayed_block_rsv_reserved, delayed_block_rsv_reserved_show);
...
static const struct attribute *allocation_attrs[] = {
	...
	BTRFS_ATTR_PTR(allocation, delayed_block_rsv_reserved),
	BTRFS_ATTR_PTR(allocation, delayed_block_rsv_size),
	NULL,
};
